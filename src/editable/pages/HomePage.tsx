...
import {
  EditableHomeHero,
  EditableCollectionsMarquee,
  EditableFeatureLoop,
  EditableCuratorGrid,
  EditableFeaturedFinds,
  EditableTimeCollections,
  EditableSocialProofBand,
...
export default async function HomePage() {
  const primaryTask = (SITE_CONFIG.tasks.find(
  ...
  const timeSections: HomeTimeSection[] = await fetchHomeTimeSections(primaryTask, {
    limit: 8,
    timeoutMs: 2500,
  })
    ...
    <EditableSiteShell>
      <main>
        ...
        <div className="mx-auto max-w-[var(--editable-container)] px-5 sm:px-8 lg:px-10">
          <Ads slot="header" showLabel eager className="mx-auto my-8 w-full" />
        </div>
        <EditableCollectionsMarquee />
        <EditableFeatureLoop />
        <EditableCuratorGrid />
        <EditableFeaturedFinds primaryTask={primaryTask} primaryRoute={primaryRoute} posts={primaryPosts} />
        <EditableTimeCollections
          primaryTask={primaryTask}
          primaryRoute={primaryRoute}
          posts={primaryPosts}
